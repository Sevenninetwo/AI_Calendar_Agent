...
# Railway deployment config (auto-injected by Railway except RAILWAY_API_TOKEN)
...
def _save_token_to_railway(creds_json: str) -> None:
    """Upsert GOOGLE_TOKEN_JSON as a Railway service variable via GraphQL API."""
    mutation = """
    ...
    logger.info("Token persisted to Railway environment variable.")


def _save_token(creds) -> None:
    """Persist refreshed credentials: Railway env var if on Railway, else disk."""
    creds_json = creds.to_json()
    if RAILWAY_API_TOKEN and RAILWAY_SERVICE_ID:
        try:
            _save_token_to_railway(creds_json)
            # Keep in-process env var fresh so the current session uses the new token
            os.environ["GOOGLE_TOKEN_JSON"] = creds_json
            return
        except Exception as e:
            logger.error("Failed to persist token to Railway: %s", e)
    # Fallback: local file (Mac / non-Railway)
...
async def _proactive_token_refresh(context: ContextTypes.DEFAULT_TYPE) -> None:
    """Refresh the Google OAuth token proactively so it never expires mid-session."""
    try:
        get_calendar_service()
        logger.info("Proactive token refresh completed.")
    except Exception as e:
        logger.error("Proactive token refresh failed: %s", e)
...
def main():
    ...
    # Refresh token every 45 minutes; first run after 60 seconds so startup settles first
    app.job_queue.run_repeating(_proactive_token_refresh, interval=45 * 60, first=60)
